...
function obSearchSortPrice($obSearch,$PAGE_RESULT_COUNT) 
{
	...
	//$new_obSearch->NavStart($PAGE_RESULT_COUNT , false);



         $arResultAll = array();
         // создаем новый массив с полученой сортировкой товара
         foreach ($new_obSearch->arResult as $arRes)
		 {
			 $arRes["SORT"] = 1;
			 //$arRes["SORT"] = rand(5, 1500);


			$search_ID = strval($arRes["ITEM_ID"]);
			if(substr( $search_ID, 0, 1 ) === "S")
			{
				//get only Sections
				$search_ID = mb_substr($search_ID, 1);
			...
			if(is_int($search_ID))
			{
				$arFilter = Array("IBLOCK_ID"=>IntVal($arRes["PARAM2"]), "ID"=>IntVal($search_ID));
			}
			else
			{
				//$arFilter = Array("IBLOCK_ID"=>IntVal($arRes["PARAM2"]), "ID"=>IntVal(66466));
			}
			 //$arFilter = Array("IBLOCK_ID"=>IntVal($arRes["PARAM2"]), "ID"=>IntVal($search_ID));
			 //$arFilter = Array("IBLOCK_ID"=>IntVal($arRes["PARAM2"]), "ID"=>IntVal(66466));
			...
			{
				$arRes["SORT"] = $arSection["SORT"];
				//$arRes["SORT"] = $search_ID;
			}
			 else
			{
				$arRes["SORT"] = 1;
...
// обнул€ем ключи нашего массива
$arResultAll = array_values($arResultAll);


// замен€ю выводимый результат
$obSearch->NavStart($PAGE_RESULT_COUNT + 1, false);


         foreach ($obSearch->arResult as $key=>$value)
		 {
			$obSearch->arResult[$key] = $arResultAll[$key-1];
         }
...
//----------- сортировка по цене
if ($arResult["REQUEST"]["HOW"] == '') 
{

	$obSearch = obSearchSortPrice($obSearch, $arParams["PAGE_RESULT_COUNT"]);
	//$obSearch->NavStart($arParams["PAGE_RESULT_COUNT"], false);
	//$arResult["DROPDOWN"] = $obSearch;
